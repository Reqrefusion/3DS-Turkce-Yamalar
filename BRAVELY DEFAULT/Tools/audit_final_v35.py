...
from bravely_font_tools_v35 import verify_font_archive

ROOT=Path(sys.argv[1])
ROMFS=ROOT/'romfs'
report={'version':'v3.5','errors':[],'warnings':[],'archive_pairs':0,'archive_entries':0,'darc_entries_validated':0,'btbf_entries_validated':0,'font':{},'romfs_file_count':0,'unexpected_source_files':[],'sha256':[]}
...
# Direct validation of BOTH runtime font archives.
report['fonts']={}
for frel in ('Graphics/UI/Font/Font','Graphics/UI_en/Font/Font'):
    font=ROMFS/frel
    if not font.is_file():
        report['errors'].append({'path':frel,'error':'font patch missing'}); continue
    try:
        vr=verify_font_archive(font.read_bytes())
        report['fonts'][frel]=vr
    except Exception as e:
        report['errors'].append({'path':frel,'error':'font active-chain verification','detail':str(e)})
...
(ROOT/'Reports/TECHNICAL_AUDIT_v35.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf8')
