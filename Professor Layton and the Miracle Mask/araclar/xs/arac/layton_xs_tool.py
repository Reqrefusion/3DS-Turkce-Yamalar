...
# CP932'in kullanıcı tanımlı (PUA) bölgesindeki ardışık kodlar.  Eşleşen
# Level-5 .xf font yamasında bu SJIS kodlarına Türkçe glifler atanmalıdır.
# Böylece XSCR biçimini veya ExeFS kodunu değiştirmeden gerçek Türkçe harfler
# gösterilebilir.
TURKISH_FONT_MAP = {
    "Ç": "\ue000",  # F0 40
    "ç": "\ue001",  # F0 41
...
TURKISH_FONT_TRANSLATION = str.maketrans(TURKISH_FONT_MAP)
...
PUNCTUATION_FONT_MAP = str.maketrans({
    "‘": "'", "’": "'", "‚": "'",
    "“": '"', "”": '"', "„": '"',
    "–": "-", "—": "-", " ": " ",
})
...
def encode_translation(text: str, policy: str) -> tuple[str, bytes, list[tuple[str, str]]]:
    ...
    if policy == "turkish-font":
        translated = text.translate(TURKISH_FONT_TRANSLATION).translate(PUNCTUATION_FONT_MAP)
        changes = [
            (before, after)
        ...
        try:
            return translated, translated.encode(TEXT_ENCODING), changes
        except UnicodeEncodeError as exc:
            problem = translated[exc.start : exc.end]
            raise XsError(
                f"turkish-font sonrasında CP932 ile kodlanamayan karakter {problem!r}."
            ) from exc
...
def command_selftest(_args: argparse.Namespace) -> dict[str, object]:
    ...
    normalized_tr, encoded_tr, _ = encode_translation(
        "ÇçĞğİıÖöŞşÜüÂâÎîÛû", "turkish-font"
    )
    expected_tr = bytes.fromhex("f040f041f042f043f044f045f046f047f048f049f04af04bf04cf04df04ef04ff050f051")
    if encoded_tr != expected_tr or normalized_tr.encode(TEXT_ENCODING) != expected_tr:
        raise XsError("Türkçe font/CP932 özel kod eşlemesi öz sınaması başarısız.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    inject_parser.add_argument(
        "--encoding-policy", choices=["strict", "turkish-ascii", "turkish-font"], default="strict"
    )
