...
class CFNTMetrics:
    """Minimal version-3 CFNT metric reader (CMAP + CWDH only)."""
    def __init__(self, data: bytes):
        if data[:4] != b'CFNT':
            raise ValueError('not CFNT')
        bom = data[4:6]
        ...
        e = self.endian
        self.default_char_width = data[finf+0x0E]  # FINF + 0x0C is default left,glyph,char
    ...
    def char_width(self, ch: str) -> int:
        gi = self.glyph_by_code.get(ord(ch))
        if gi is None:
